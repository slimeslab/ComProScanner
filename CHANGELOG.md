...
- Fixed DOI-to-folder-name lookups in `RAGTool`, `GraphExtractorTool`, `EquationTool`, and `DataExtractionFlow` to also replace `:` with `_` (not just `/`), so DOIs like `10.1023/A:1004776418521` correctly resolve to their saved figure/vector-DB directories.
